...
class Mamba2LMHeadModel(nn.Module):
    ...
    def forward(
        ...
        self, input_ids: LongTensor, h: Optional[Union[List[InferenceCache], List[None]]] = None
...
class Mamba2_D(nn.Module):
    def __init__(self, args: Mamba2Config, device: Device = None):
        ...
        # Order: (z, x, B, C, dt)
        d_in_proj = 0 * args.d_inner + 2 * args.d_state + args.nheads
        self.in_proj = nn.Linear(args.d_model, d_in_proj, bias=False, device=device)
        self.in_proj_z = nn.Linear(args.d_model, args.d_inner, bias=False, device=device)
        self.in_proj_z2 = nn.Linear(args.d_model, args.d_inner, bias=False, device=device)
        conv_dim = 0 * args.d_inner + 2 * args.d_state
    ...
    # def forward(self, u: Tensor, z: Tensor, z2: Tensor, h: InferenceCache | None = None):
    def forward(self, u: Tensor, z: Tensor, z2: Tensor, h: Optional[InferenceCache] = None):
        """
        Arguments
        ...
        """
        if h:
            return self.step(u, z, z2, h)

        A = -torch.exp(self.A_log)  # (nheads,)
        zxbcdt = self.in_proj(u)  # (batch, seqlen, d_in_proj)
        x = silu(self.in_proj_z(z))  # (batch, seqlen, d_inner)
        # x = silu(x)
        # x = z
        z = silu(self.in_proj_z2(z2))
        xBC, dt = torch.split(
            zxbcdt,
            [
                2 * self.args.d_state,
        ...
        B, C = torch.split(
            xBC, [self.args.d_state, self.args.d_state], dim=-1
        )
    ...
    # def step(self, u: Tensor, z: Tensor, z2: Tensor, h: InferenceCache) -> tuple[Tensor, InferenceCache]:
    def step(self, u: Tensor, z: Tensor, z2: Tensor, h: InferenceCache) -> Tuple[Tensor, InferenceCache]:
        ...
        # z = self.in_proj_z(z)  # (batch, seqlen, d_inner)
        x = z
        z = z2
        xBC, dt = torch.split(
            zxbcdt,
...
class Mamba2(nn.Module):
    ...
    def forward(self, u: Tensor, h: Optional[InferenceCache] = None):
        """
        Arguments
            u: (batch, seqlen, d_model) input. seqlen should be a multiple of chunk_size.
            h: hidden states for inference step. Initialized to 0s if not present.
