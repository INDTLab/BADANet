    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            if i != self.num_stages - 1:
                ...
                # 最后一层保留 token 格式
                x = self.norm(x)
                # 去掉 cls_token，只保留 patch tokens，再 reshape 回 feature map
                feat = x[:, 1:, :].reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
                outs.append(feat)

        return outs  # 返回4个尺度的特征 (C2, C3, C4, C5)

    def forward(self, x):
        x = self.forward_features(x)
        # x = self.head(x) # 只用backbone，注释掉这里
