...
class mdASPP(nn.Module):
    def __init__(self, in_dim, out_dim, dilations=[1,2,3,5,7]):
        super(mdASPP, self).__init__()

        # stage1: sequential dilations
        self.conv01 = ConvBNReLU(in_dim, out_dim, 3, 1, dilations[0], dilation=dilations[0])
        self.conv02 = ConvBNReLU(out_dim, out_dim, 3, 1, dilations[1], dilation=dilations[1])
        self.conv03 = ConvBNReLU(out_dim, out_dim, 3, 1, dilations[2], dilation=dilations[2])
        self.conv04 = ConvBNReLU(out_dim, out_dim, 3, 1, dilations[3], dilation=dilations[3])
        self.conv05 = ConvBNReLU(out_dim, out_dim, 3, 1, dilations[4], dilation=dilations[4])

        # stage2: parallel dilations
        self.conv1 = ConvBNReLU(out_dim*5, out_dim, 3, 1, dilations[0], dilation=dilations[0])
        self.conv2 = ConvBNReLU(out_dim*5, out_dim, 3, 1, dilations[1], dilation=dilations[1])
        self.conv3 = ConvBNReLU(out_dim*5, out_dim, 3, 1, dilations[2], dilation=dilations[2])
        self.conv4 = ConvBNReLU(out_dim*5, out_dim, 3, 1, dilations[3], dilation=dilations[3])
        self.conv5 = ConvBNReLU(out_dim*5, out_dim, 3, 1, dilations[4], dilation=dilations[4])

        self.fuse = ConvBNReLU(out_dim * 5, out_dim, 3, 1, 1)
...
class CAMO(BasicModelClass):
    def __init__(self):
        ...
        # # 原版
        # self.d5 = mdASPP(64, 64, dilations=[1,2,3,5,7])
        # self.d4 = mdASPP(64, 64, dilations=[1,2,3,5,7])
        # self.d3 = mdASPP(64, 64, dilations=[1,2,3,5,7])
        # self.d2 = mdASPP(64, 64, dilations=[1,2,3,5,7])
        # self.d1 = mdASPP(64, 64, dilations=[1,2,3,5,7])
        # self.d0 = mdASPP(64, 64, dilations=[1,2,3,5,7])

        # 膨胀率全设为 1，相当于只用普通3x3卷积
        # self.d5 = mdASPP(64, 64, dilations=[1, 1, 1, 1, 1])
        # self.d4 = mdASPP(64, 64, dilations=[1, 1, 1, 1, 1])
        # self.d3 = mdASPP(64, 64, dilations=[1, 1, 1, 1, 1])
        # self.d2 = mdASPP(64, 64, dilations=[1, 1, 1, 1, 1])
        # self.d1 = mdASPP(64, 64, dilations=[1, 1, 1, 1, 1])
        # self.d0 = mdASPP(64, 64, dilations=[1, 1, 1, 1, 1])

        # 膨胀率全设为 3，测试单一尺度膨胀效果
        # self.d5 = mdASPP(64, 64, dilations=[3, 3, 3, 3, 3])
        # self.d4 = mdASPP(64, 64, dilations=[3, 3, 3, 3, 3])
        # self.d3 = mdASPP(64, 64, dilations=[3, 3, 3, 3, 3])
        # self.d2 = mdASPP(64, 64, dilations=[3, 3, 3, 3, 3])
        # self.d1 = mdASPP(64, 64, dilations=[3, 3, 3, 3, 3])
        # self.d0 = mdASPP(64, 64, dilations=[3, 3, 3, 3, 3])

        # 膨胀率=[1,3,6,12,18]
        # self.d5 = mdASPP(64, 64, dilations=[1,3,6,12,18])
        # self.d4 = mdASPP(64, 64, dilations=[1,3,6,12,18])
        # self.d3 = mdASPP(64, 64, dilations=[1,3,6,12,18])
        # self.d2 = mdASPP(64, 64, dilations=[1,3,6,12,18])
        # self.d1 = mdASPP(64, 64, dilations=[1,3,6,12,18])
        # self.d0 = mdASPP(64, 64, dilations=[1,3,6,12,18])

        # 膨胀率=[1,3,6,12,18]
        self.d5 = mdASPP(64, 64, dilations=[1,2,3,4,5])
        self.d4 = mdASPP(64, 64, dilations=[1,2,3,4,5])
        self.d3 = mdASPP(64, 64, dilations=[1,2,3,4,5])
        self.d2 = mdASPP(64, 64, dilations=[1,2,3,4,5])
        self.d1 = mdASPP(64, 64, dilations=[1,2,3,4,5])
        self.d0 = mdASPP(64, 64, dilations=[1,2,3,4,5])
