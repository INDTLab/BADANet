...
class Focus1(nn.Module):
    ...
    def forward(self, x, mask=None, edge=None, x_e=None, back=None):
        ...
        b0 = refine1 * (1 - mask)  # (b,64,16,16), no_Boundary消融
...
class CAMO(BasicModelClass):
    ...
    def body(self, m_scale):
        ...
        x = self.a5(x=feats[0], mask=x6.sigmoid(), edge=e6.sigmoid(), x_e=e60) # BADA
        x = cus_sample(x, mode="scale", factors=2)

        x5 = self.out_layer_05(x)
        x = self.a4(x=self.add4(torch.cat((feats[1], x), 1)), mask=x5.sigmoid(), edge=e5.sigmoid(), x_e=e50)
        x = cus_sample(x, mode="scale", factors=2)

        x4 = self.out_layer_04(x)
        x = self.a3(x=self.add3(torch.cat((feats[2], x), 1)), mask=x4.sigmoid(), edge=e4.sigmoid(), x_e=e40)
        x = cus_sample(x, mode="scale", factors=2)

        x3 = self.out_layer_03(x)
        x = self.a2(x=self.add2(torch.cat((feats[3], x), 1)), mask=x3.sigmoid(), edge=e3.sigmoid(), x_e=e30)
        x = cus_sample(x, mode="scale", factors=2)

        x2 = self.out_layer_02(x)
        x = self.a1(x=self.add1(torch.cat((feats[4], x), 1)), mask=x2.sigmoid(), edge=e2.sigmoid(), x_e=e20)
        ...
        ''' no BADA 
        x6 = self.out_layer_06(feats[0]) # (b,1,16,16)
        x5 = self.out_layer_06(feats[0]) # (b,1,16,16)
        x4 = self.out_layer_06(feats[1]) # (b,1,32,32)
        x3 = self.out_layer_06(feats[2]) # (b,1,64,64)
        x2 = self.out_layer_06(feats[3]) # (b,1,128,128)
        x1 = self.out_layer_05(feats[4]) # (b,1,256,256)
        '''

        ''' w/o Boundary & w/ BADA, 具体逻辑在Focus1类中 '''
        x6 = self.out_layer_06(feats[0])  # (b,1,16,16)
        x = self.a5(x=feats[0], mask=x6.sigmoid(), edge=e6.sigmoid(), x_e=e60)  # BADA
        x = cus_sample(x, mode="scale", factors=2)
        ...
        ''' w/o Boundary & BADA -> FM 记得把self.a1~a5换回来
        x6 = self.out_layer_06(feats[0])  # (b,1,16,16)
        # x = self.a5(x=feats[0], mask=x6.sigmoid(), edge=e6.sigmoid(), x_e=e60)  # BADA
