...
class dASPP(nn.Module):
    ...
    def forward(self, x): # x: b_3^1

        x1 = self.conv01(x) # f_1^{p0}, (b,64,64,64)
        x2 = self.conv02(x1) # f_2^{p0}, (b,64,64,64)
        x3 = self.conv03(x2) # f_3^{p0}, (b,64,64,64)
        x4 = self.conv04(x3) # f_4^{p0}, (b,64,64,64)
        x5 = self.conv05(x4) # f_5^{p0}, (b,64,64,64)

        conv1 = self.conv1(torch.cat((x1, x2, x3, x4, x5), 1)) # f_1^{p1}, (b,64,64,64)
        conv2 = self.conv2(torch.cat((conv1, x2, x3, x4, x5), 1)) # f_2^{p1}, (b,64,64,64)
        conv3 = self.conv3(torch.cat((conv1, conv2, x3, x4, x5), 1)) # f_3^{p1}, (b,64,64,64)
        conv4 = self.conv4(torch.cat((conv1, conv2, conv3, x4, x5), 1)) # f_4^{p1}, (b,64,64,64)
        conv5 = self.conv5(torch.cat((conv1, conv2, conv3, conv4, x5), 1)) # f_5^{p1}, (b,64,64,64)

        out = self.fuse(torch.cat((conv1, conv2, conv3, conv4, conv5), 1)) # b_3^p
        return out
...
class Focus1(nn.Module):
    ...
    def forward(self, x, mask=None, edge=None, x_e=None, back=None):
        ...
        refine1 = self.conv33(x + e3) # R_1 (b,64,16,16)
        ...
        b3 = self.a2(b1, b2, b4) #A_{bg} (b,64,16,16)
        # print(b3.shape)
        ...
        refine3_0 = f5 + f3 # (b,64,16,16)
        refine3 = self.conv1(refine3_0) # (b,64,16,16)
...
class Attention1(nn.Module):
    ...
    def forward(self, x_0, x_1, x, mask=None, e0=None, e1=None):
        ...
        ''' '''
        # plt.imshow(attn1[0, 0, :, :].detach().cpu().numpy())
        # plt.savefig("/data1/wuhu/Projects/CAMO_xiao/output/CAMO/tmp/tmp1.png")
        v = attn1 @ v
...
class NeighborConnectionDecoder40(nn.Module):
    ...
    def forward(self, zt5, zt4, zt3, zt2, zt1, depth=None):
        zt1_1 = zt1 # f_1^t, (b,64,256,256)
        zt2_1 = self.conv_concat2(torch.cat((self.downsample(zt1_1), zt2), dim=1)) # f_2^{m0}, (b,128,128,128)
        zt3_1 = self.conv_concat3(torch.cat((self.downsample(zt2_1), zt3), dim=1)) # f_3^{m0}, (b,192,64,64)
        zt4_1 = self.conv_concat4(torch.cat((self.downsample(zt3_1), zt4), dim=1)) # f_4^{m0}, (b,256,32,32)
        zt5_1 = self.conv_concat5(torch.cat((self.downsample(zt4_1), zt5), dim=1)) # f_5^{m0}, (b,320,16,16)


        zt5_2 = self.conv_upsample1(zt5_1) # f_5^{m}, (b,64,16,16)

        zt4_1 = torch.cat((zt4_1, self.conv_upsample6(self.upsample(zt5_1))), 1)
        zt4_2 = self.conv_upsample2(zt4_1) # f_4^{m}, (b,64,32,32)

        zt3_1 = torch.cat((zt3_1, self.conv_upsample7(self.upsample(zt4_1))), 1)
        zt3_2 = self.conv_upsample3(zt3_1) # f_3^{m}, (b,64,64,64)

        zt2_1 = torch.cat((zt2_1, self.conv_upsample8(self.upsample(zt3_1))), 1)
        zt2_2 = self.conv_upsample4(zt2_1) # f_2^{m}, (b,64,128,128)

        zt1_1 = torch.cat((zt1_1, self.conv_upsample9(self.upsample(zt2_1))), 1)
        zt1_2 = self.conv_upsample5(zt1_1) # f_1^{m}, (b,64,256,256)
...
class NeighborConnectionDecoder1(nn.Module):
    ...
    def forward(self, zt5, zt4, zt3, zt2, zt1): # zt5~1对应论文中的ft5~1

        zt5_1 = self.conv_upsample0(zt5) * zt5 # zt5_1: b_5^0, (b,64,16,16)
        zt1_1 = self.conv_upsample00(zt1) * zt1 # zt1_1: b_1^0, (b,64,256,256)
        zt4_1 = (self.conv_upsample000(self.upsample(zt5_1)) * zt4) # b_4^0, (b,64,32,32)
        zt2_1 = (self.conv_upsample4(self.downsample(zt1_1)) * zt2) # b_2^0, (b,64,128,128)
        zt3_1 = (self.conv_upsample2(self.upsample(zt4_1)) * self.conv_upsample3(self.downsample(zt2_1)) * zt3) # b_3^0, (b,64,64,64)

        zt4_2 = torch.cat((zt4_1, self.conv_upsample9(self.upsample(zt5_1))), 1)
        zt4_2 = self.conv_concat2(zt4_2) # b_4^1, (b,64,32,32)

        zt2_2 = torch.cat((zt2_1, self.conv_upsample11(self.downsample(zt1_1))), 1)
        zt2_2 = self.conv_concat4(zt2_2) # b_2^1, (b,64,128,128)

        zt3_2 = torch.cat(
            (self.conv_upsample12(self.upsample(zt4_2)), zt3_1, self.conv_upsample13(self.downsample(zt2_2))), 1)
        zt3_2 = self.conv_concat3(zt3_2) # b_3^1, (b,64,64,64)
...
class CAMO(BasicModelClass):
    ...
    def body(self, m_scale, batch):
        ...
        # 直接可视化，不resize
        # visualize_edges([e6, e5, e4, e3, e2], titles=["16x16", "32x32", "64x64", "128x128", "256x256"])
        # 如果想全部resize到256x256统一大小再看
        # visualize_edges([e6, e5, e4, e3, e2], titles=["16x16", "32x32", "64x64", "128x128", "256x256"], resize_to=(256, 256))
        # visualize_edges([edge,], titles=["64x64",], resize_to=(256, 256))
        # visualize_edge(edge, title="CBR Output Edge (64x64)")
        ...
        '''
        # image = cv2.imread(batch['info']['mask_path'][0])  # (H,W,3)
        # image[:] = 1  # 所有像素置为0
        # visualize_multiscale_feats(image, feats, alpha=0.6, mode='mean')
        ...
        # visualize_edge(edge, title="CBR Output Edge (64x64)")
        # visualize_edge_boundary(edge, title="CBR Output Edge (64x64)")
...
def feature_to_heatmap(feat, image_shape, mode='mead'):
    """
    feat: (C,H,W) torch.Tensor
    image_shape: (H_img, W_img)
    """
    if isinstance(mode, int):  # 指定通道
        fmap = feat[mode].detach().cpu().numpy()
    elif mode == "mean":
        fmap = feat.mean(dim=0).detach().cpu().numpy()
    elif mode == "max":
        fmap = feat.max(dim=0)[0].detach().cpu().numpy()
    else:
        raise ValueError("mode must be 'mean', 'max' or int")
    # fmap = feat.mean(dim=0).detach().cpu().numpy()   # (H,W)
    fmap = cv2.resize(fmap, (image_shape[1], image_shape[0]))
...
def visualize_multiscale_feats(image, feats, alpha=1.0, mode='mean'):
    """
    image: numpy 原图 (H,W,3), BGR
    feats: list of 5 feature maps [(B,C,H,W), ...]
    """
    vis_list = []
    for i, feat in enumerate(feats):
        feat = feat[0]  # batch=0
        heatmap, fmap = feature_to_heatmap(feat, image.shape[:2], mode=mode)
        overlay = overlay_heatmap(image, heatmap, alpha=alpha)
        vis_list.append(overlay[..., ::-1])  # 转成RGB方便matplotlib显示

    # 拼接展示
    fig, axes = plt.subplots(1, len(vis_list), figsize=(20,5))
    for i, overlay in enumerate(vis_list):
        axes[i].imshow(overlay)
        axes[i].set_title(f"Feat {i+1} ({feats[i].shape[2]}x{feats[i].shape[3]})")
        axes[i].axis("off")
    plt.tight_layout()
    # plt.show()
    plt.savefig("/data1/wuhu/Projects/CAMO_xiao/output/CAMO/tmp/tmp1.png")

def visualize_edges(edge_list, titles=None, resize_to=None):
    """
    edge_list: list of edge tensors [(B,1,H,W), ...]
    titles: list of str, 每个子图标题
    resize_to: (H,W)，是否把所有边缘图resize到统一大小
    """
    num_edges = len(edge_list)
    fig, axes = plt.subplots(1, num_edges, figsize=(4*num_edges, 4))

    for i, edge in enumerate(edge_list):
        edge = edge[0,0].detach().cpu().numpy()  # (H,W)
        edge = (edge - edge.min()) / (edge.max() - edge.min() + 1e-5)  # 归一化

        if resize_to is not None:
            edge = cv2.resize(edge, (resize_to[1], resize_to[0]))
...
def visualize_edge(edge, title="Edge Feature"):
    """
    edge: (B,1,H,W) torch.Tensor
    """
    # 取 batch=0, channel=0
    edge_np = edge[0,0].detach().cpu().numpy()

    # 归一化到0-1
    edge_np = (edge_np - edge_np.min()) / (edge_np.max() - edge_np.min() + 1e-5)
...
def visualize_edge_boundary(edge, title="Edge Boundary"):
    """
    edge: (B,1,H,W) torch.Tensor
    title: 图片标题
    """
    ...
    # Sobel 计算梯度（绝对值累加）
    grad_x = cv2.Sobel(edge_np, cv2.CV_64F, 1, 0, ksize=3)
    grad_y = cv2.Sobel(edge_np, cv2.CV_64F, 0, 1, ksize=3)
    boundary = np.abs(grad_x) + np.abs(grad_y)  # 绝对值累加
    boundary = (boundary / boundary.max())  # 归一化到0-1
...
# def normalize_map(fmap):
#     """归一化到0-1"""
#     return (fmap - fmap.min()) / (fmap.max() - fmap.min() + 1e-5)
#
# def feature_to_heatmap(feat, mode="mean"):
#     """
#     feat: (C,H,W) torch.Tensor
#     mode: "mean" | "max" | int (通道号)
#     """
#     if isinstance(mode, int):  # 指定通道
#         fmap = feat[mode].detach().cpu().numpy()
#     elif mode == "mean":
...
#
# def visualize_multiscale_features(feats, mode="mean"):
#     """
#     feats: list of feature maps [(B,C,H,W), ...]
#     mode: "mean" | "max" | int (展示方式)
#     """
...
#     for i, feat in enumerate(feats):
#         feat = feat[0]  # batch=0
#         fmap = feature_to_heatmap(feat, mode=mode)
#         axes[i].imshow(fmap, cmap="jet")
#         axes[i].set_title(f"Scale {feat.shape[1]}x{feat.shape[2]}")
#         axes[i].axis("off")
#
#     plt.tight_layout()
#     # plt.show()
