...
class CAMO(BasicModelClass):
    def __init__(self):
        ...
        # DASPP
        # self.d5 = DenseASPP(64, 64)
        # self.d4 = DenseASPP(64, 64)
        # self.d3 = DenseASPP(64, 64)
        # self.d2 = DenseASPP(64, 64)
        # self.d1 = DenseASPP(64, 64)
        # self.d0 = DenseASPP(64, 64)
...
class DenseASPP(nn.Module):
    def __init__(self, in_channels, inter_channels=64, dilation_rates=[3, 6, 12, 18, 24]):
        ...
        for i, d in enumerate(dilation_rates):
            # 每个卷积的输入通道 = in_channels + i * inter_channels
            self.layers.append(
                ConvBNReLU(in_channels + i * inter_channels, inter_channels, kernel_size=3,
                           padding=d, dilation=d)
            )
        # 融合卷积
        self.fuse = ConvBNReLU(in_channels + len(dilation_rates) * inter_channels, in_channels, kernel_size=1,
                               padding=0)
...
class ASPP(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, atrous_rates=[6, 12, 18]):
        ...
        # 1x1 卷积分支
        self.branches.append(ConvBNReLU(in_channels, out_channels, kernel_size=1, padding=0))
        ...
        # 融合卷积
        self.fuse = ConvBNReLU(len(self.branches) * out_channels + out_channels, out_channels, kernel_size=1,
                               padding=0)

    def forward(self, x):
        size = x.shape[2:]  # 高宽
        res = [branch(x) for branch in self.branches]  # 前四个分支
