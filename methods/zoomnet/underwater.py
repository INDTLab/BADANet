...
class TransLayer(nn.Module):
    def __init__(self, out_c, last_module=dASPP):
        ...
        self.c5_down = nn.Sequential(
            # ConvBNReLU(2048, out_c, 3, 1, 1),
            # ConvBNReLU(768, out_c, 3, 1, 1),
            # last_module(in_dim=2048, out_dim=out_c),
            # last_module(in_dim=512, out_dim=out_c),
            # last_module(in_dim=1024, out_dim=out_c),
            # last_module(in_dim=584, out_dim=out_c),
            # last_module(in_dim=304, out_dim=out_c),
            # last_module(in_dim=448, out_dim=out_c),
            # last_module(in_dim=320, out_dim=out_c),
            # last_module(512, out_c),
            ConvBNReLU(512, out_c, 3, 1, 1),
            # MBSA('cuda:1', 512, out_c),
            # nn.ReLU()
            # dASPP(out_c, out_c)
            # dASPP(512, out_c),
            # MBConvBlock((512 + 1024), out_c),
            # MBConvBlock(512, out_c),
            # dASPP(512, out_c)
        )
        # self.c4_down = nn.Sequential(ConvBNReLU(160, out_c, 3, 1, 1))
        # self.c3_down = nn.Sequential(ConvBNReLU(96, out_c, 3, 1, 1))
        # self.c2_down = nn.Sequential(ConvBNReLU(48, out_c, 3, 1, 1))
        # self.c1_down = nn.Sequential(ConvBNReLU(48, out_c, 3, 1, 1))

        # self.c4_down = nn.Sequential(ASPP4(160, out_c))
        # self.c3_down = nn.Sequential(ASPP3(96, out_c))
        # self.c2_down = nn.Sequential(ASPP2(48, out_c))
        # self.c1_down = nn.Sequential(ASPP1(48, out_c))
        # self.c4_down = nn.Sequential(MBConvBlock(160, out_c))
        # self.c3_down = nn.Sequential(MBConvBlock(80, out_c))
        # self.c2_down = nn.Sequential(MBConvBlock(40, out_c))
        # self.c1_down = nn.Sequential(MBConvBlock(40, out_c))

        # self.c4_down = nn.Sequential(MBConvBlock(288, out_c))
        # self.c3_down = nn.Sequential(MBConvBlock(160, out_c))
        # self.c2_down = nn.Sequential(MBConvBlock(80, out_c))
        # self.c1_down = nn.Sequential(MBConvBlock(80, out_c))

        # self.c4_down = nn.Sequential(MBConvBlock(512, out_c))
        # self.c3_down = nn.Sequential(MBConvBlock(256, out_c))
        # self.c2_down = nn.Sequential(MBConvBlock(128, out_c))
        # self.c1_down = nn.Sequential(MBConvBlock(128, out_c))

        self.c4_down = nn.Sequential(
            # MBConvBlock((320 + 512), out_c),
            # ConvBNReLU(1024, out_c, 3, 1, 1),
            # ConvBNReLU(384, out_c, 3, 1, 1),
            ConvBNReLU(320, out_c, 3, 1, 1),
            # MBSA('cuda:1', 320, out_c)
            # dASPP(320, out_c),
        )
        self.c3_down = nn.Sequential(
            # MBConvBlock((128 + 256), out_c),
            # ConvBNReLU(512, out_c, 3, 1, 1),
            # ConvBNReLU(192, out_c, 3, 1, 1),
            ConvBNReLU(128, out_c, 3, 1, 1),
            # MBSA('cuda:1', 128, out_c)
            # dASPP(128, out_c),
        )
        self.c2_down = nn.Sequential(
            # MBConvBlock((64 + 128), out_c),
            # ConvBNReLU(256, out_c, 3, 1, 1),
            # ConvBNReLU(96, out_c, 3, 1, 1),
            ConvBNReLU(64, out_c, 3, 1, 1),
            # MBSA('cuda:1', 64, out_c)
            # dASPP(64, out_c),
        )
        self.c1_down = nn.Sequential(
            # MBConvBlock((64 + 128), out_c),
            # MBConvBlock(64, out_c),
            # ConvBNReLU(96, out_c, 3, 1, 1),
            ConvBNReLU(64, out_c, 3, 1, 1),
            # MBSA('cuda:1', 64, out_c)
            # dASPP(64, out_c),
...
@MODELS.register()
class UnderWater(BasicModelClass):
    def __init__(self):
        super().__init__()
        ...
        self.ncd2 = NeighborConnectionDecoder402(64)
    ...
    def body(self, m_scale, depth=None):
        ...
        # m_trans_feats = self.br1(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        # m_trans_feats1 = list(m_trans_feats1)
        # r_trans_feats = self.encoder_translayer1(r_scale)
        # s_trans_feats = self.encoder_translayer(s_scale)
        feats1 = m_trans_feats
        feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        feats2 = feats
        #
        # feats[0] = self.d5(feats[0])
        # feats[1] = self.d4(feats[1])
        # feats[2] = self.d3(feats[2])
        # feats[3] = self.d2(feats[3])
        # feats[4] = self.d1(feats[4])
        #
        # edge3 = self.ncd1(feats[0], feats[1], feats[2], feats[3], feats[4])
        # cus_sample(x, mode="scale", factors=2)
        # e10 = cus_sample(m_trans_feats[0], mode="scale", factors=32)
        # e20 = cus_sample(m_trans_feats[1], mode="scale", factors=16)
        # e30 = cus_sample(m_trans_feats[2], mode="scale", factors=8)
        # e40 = cus_sample(m_trans_feats[3], mode="scale", factors=4)
        # e50 = cus_sample(m_trans_feats[4], mode="scale", factors=2)

        # edges = self.ncd1(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        #
        # e6 = self.out_layer_06e(edges[0])
        ...
        # e20 = edges[4]
        #
        # # edge1, edge2, edge3, edge4, edge5 = self.ncd1(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        # # # edge5 = self.ncd1(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        # #
        # # # edge0 = self.d0(edge3)
        # # edge0 = edge5
        # # # edge0 = self.mc(self.m0(edge3, edge3))
        # #
        # # edge = self.bout_layer_00(edge0)
        # #
        # # e6 = cus_sample(edge, mode="size", factors=m_trans_feats[0].shape[2:])
        # # e5 = cus_sample(edge, mode="size", factors=m_trans_feats[1].shape[2:])
        # # e4 = cus_sample(edge, mode="size", factors=m_trans_feats[2].shape[2:])
        # # e3 = cus_sample(edge, mode="size", factors=m_trans_feats[3].shape[2:])
        # # e2 = cus_sample(edge, mode="size", factors=m_trans_feats[4].shape[2:])
        # #
        # # e60 = cus_sample(edge0, mode="size", factors=m_trans_feats[0].shape[2:])
        # # e50 = cus_sample(edge0, mode="size", factors=m_trans_feats[1].shape[2:])
        # # e40 = cus_sample(edge0, mode="size", factors=m_trans_feats[2].shape[2:])
        # # e30 = cus_sample(edge0, mode="size", factors=m_trans_feats[3].shape[2:])
        # # e20 = cus_sample(edge0, mode="size", factors=m_trans_feats[4].shape[2:])
        #
        # feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        #
        ...
        #
        # # feats[0] = self.d5(feats[0] + edge1)
        # # feats[1] = self.d4(feats[1] + edge2)
        # # feats[2] = self.d3(feats[2] + edge3)
        # # feats[3] = self.d2(feats[3] + edge4)
        # # feats[4] = self.d1(feats[4] + edge5)
        #
        # # feats = []
        # # for m, layer in zip(m_trans_feats, self.merge_layers):
        # #     siu_outs = layer(m)
        # #     feats.append(siu_outs)
        # # feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        # # feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4], edge0, depth)
        # # feats = m_trans_feats
        # # feats1 = self.br2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        # # feats2 = self.br1(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        # # feats1 = m_trans_feats
        # # feats2 = m_trans_feats
        #
        #
        # # m_trans_feats[0] = self.d5(m_trans_feats[0])
        # # m_trans_feats[1] = self.d4(m_trans_feats[1])
        # # m_trans_feats[2] = self.d3(m_trans_feats[2])
        # # m_trans_feats[3] = self.d2(m_trans_feats[3])
        # # m_trans_feats[4] = self.d1(m_trans_feats[4])


        # x6_dp = self.add6d(m_trans_feats[0])
        # x6_d = self.out_layer_06d(x6_dp)
        # x6_db = self.add6db(torch.cat((self.back_layer_06d(x6_d), x6_dp), 1))
        #
        # x6_du = cus_sample(m_trans_feats[0], mode="scale", factors=2)
        #
        # x5_dp = self.add5d(torch.cat((x6_du, m_trans_feats[1]), 1))
        # x5_d = self.out_layer_05d(x5_dp)
        # x5_db = self.add5db(torch.cat((self.back_layer_05d(x5_d), x5_dp), 1))
        #
        # x5_du = cus_sample(x5_dp, mode="scale", factors=2)
        #
        # x4_dp = self.add4d(torch.cat((x5_du, m_trans_feats[2]), 1))
        # x4_d = self.out_layer_04d(x4_dp)
        # x4_db = self.add4db(torch.cat((self.back_layer_04d(x4_d), x4_dp), 1))
        #
        # x4_du = cus_sample(x4_dp, mode="scale", factors=2)
        #
        # x3_dp = self.add3d(torch.cat((x4_du, m_trans_feats[3]), 1))
        # x3_d = self.out_layer_03d(x3_dp)
        # x3_db = self.add3db(torch.cat((self.back_layer_03d(x3_d), x3_dp), 1))
        #
        # x3_du = cus_sample(x3_dp, mode="scale", factors=2)
        #
        # x2_dp = self.add2d(torch.cat((x3_du, m_trans_feats[4]), 1))
        # x2_d = self.out_layer_02d(x2_dp)
        # x2_db = self.add2db(torch.cat((self.back_layer_02d(x2_d), x2_dp), 1))
        #
        # feats = self.ncd2(x6_db, x5_db, x4_db, x3_db, x2_db)
        # x6_bp = self.add6b(m_trans_feats[0])
        # x6_d = self.out_layer_06d(x6_bp)
        #
        # x6_bu = cus_sample(m_trans_feats[0], mode="scale", factors=2)
        #
        # x5_bp = self.add5b(torch.cat((x6_bu, m_trans_feats[1]), 1))
        # x5_d = self.out_layer_05d(x5_bp)
        #
        # x5_bu = cus_sample(x5_bp, mode="scale", factors=2)
        #
        # x4_bp = self.add4b(torch.cat((x5_bu, m_trans_feats[2]), 1))
        # x4_d = self.out_layer_04d(x4_bp)
        #
        # x4_bu = cus_sample(x4_bp, mode="scale", factors=2)
        #
        # x3_bp = self.add3b(torch.cat((x4_bu, m_trans_feats[3]), 1))
        # x3_d = self.out_layer_03d(x3_bp)
        #
        # x3_bu = cus_sample(x3_bp, mode="scale", factors=2)
        #
        # x2_bp = self.add2b(torch.cat((x3_bu, m_trans_feats[4]), 1))
        # x2_d = self.out_layer_02d(x2_bp)

        # edge3 = self.ncd1(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4])
        #
        # # edge0 = self.d0(edge3)
        # edge0 = edge3
        # # edge0 = self.mc(self.m0(edge3, edge3))
        ...
        # # feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4], e5_1, e4_2, e3_2, e2_2, e1_1)
        # # feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4],
        # #                   e6, e5, e4, e3, e2)
        # feats = self.ncd2(m_trans_feats[0], m_trans_feats[1], m_trans_feats[2], m_trans_feats[3], m_trans_feats[4],
        #                   e60, e50, e40, e30, e20)

        # x6 = self.out_layer_06(feats[0])
        # x5 = self.out_layer_05(feats[1])
        # x4 = self.out_layer_04(feats[2])
        # x3 = self.out_layer_03(feats[3])
        # x2 = self.out_layer_02(feats[4])
        # x1 = self.out_layer_01(self.up1(feats[4]))

        # d6 = self.out_layer_06d(m_trans_feats[0])
        # d5 = self.out_layer_05d(m_trans_feats[1])
        # d4 = self.out_layer_04d(m_trans_feats[2])
        # d3 = self.out_layer_03d(m_trans_feats[3])
        # d2 = self.out_layer_02d(m_trans_feats[4])
        ...
        # feats = self.ncd2(feats[0], feats[1], feats[2], feats[3], feats[4])
        #
        # feats[0] = self.m5(feats[0])
        # feats[1] = self.m4(feats[1])
        # feats[2] = self.m3(feats[2])
        # feats[3] = self.m2(feats[3])
        # feats[4] = self.m1(feats[4])

        # # feats[0] = self.d5(feats[0])
        # x6 = self.out_layer_06(feats[0])
        # # e6 = self.out_layer_06e(feats[0])
        # # x6 = x6 + m6
        # # x6 = self.bout_layer_05(torch.cat((x6, e6), 1))
        # # e6 = cus_sample(edge, mode="size", factors=x6.shape[2:])
        # # e6 = self.mout_layer_00(edge[0])
        # # feats[0] = self.aa5(feats[0], x6.sigmoid(), e0.sigmoid())
        # # x = self.d5(feats[0])
        # # x = self.a5(feats[0], x6.sigmoid(), e6.sigmoid())
        # # x = self.d5(self.a5(feats[0], x6.sigmoid(), e6.sigmoid()))
        # # x = self.add5(torch.cat((feats[0], self.a5(feats[0], x6.sigmoid(), e6.sigmoid())), 1))
        # # x = self.a5(feats[0], x6.sigmoid(), feats[0], e6.sigmoid(), e60, x6_bp)
        # # x = self.a5(feats[0], x6.sigmoid())
        # # x = self.add5(feats[0])
        # # x = self.add5((feats[0] + self.a5(feats[0], x6.sigmoid(), e6.sigmoid(), e60)))
        # # x = self.a5(feats[0], x6.sigmoid(), e6.sigmoid(), e60)
        # # x = self.add5(torch.cat((feats[0], self.a5(feats[0], x6.sigmoid(), e6.sigmoid(), e60)), 1))
        # # x = feats[0]
        # x = self.a5(feats[0], x6.sigmoid(), e6.sigmoid(), e60)
        # # x = self.a5(feats[0], x6.sigmoid())
        # # x = self.d5(x)
        # # x5 = cus_sample(self.out_layer_05(x), mode="scale", factors=2)
        # x = cus_sample(x, mode="scale", factors=2)
        # # e5 = cus_sample(e6, mode="scale", factors=2)
        # x50 = cus_sample(x6, mode="scale", factors=2)
        #
        # # m5 = x
        # # x = self.a4(self.add4(torch.cat((feats[1], x), dim=1)), x50.sigmoid(), e5.sigmoid(), e50)
        # # x = self.a4((feats[1] + x), x50.sigmoid(), e5.sigmoid(), e50)
        # # x = self.add4((x + self.a4(feats[1], x50.sigmoid(), e5.sigmoid(), e50)))
        # x = self.add4(torch.cat((x, self.a4(feats[1], x50.sigmoid(), e5.sigmoid(), e50)), 1))
        # # x = self.add4(torch.cat((x, self.a4(feats[1], x50.sigmoid())), 1))
        # # x = self.add4(torch.cat((x, feats[1]), 1))
        # # x = self.add4(feats[1], x)
        # # x = self.d4(x)
        # x5 = self.out_layer_05(x)
        # # e5 = self.out_layer_05e(x)
        # # x5 = self.bout_layer_04(torch.cat((x5, e5), 1))
        # # e5 = cus_sample(edge, mode="size", factors=x5.shape[2:])
        # # e5 = self.mout_layer_01(feats[1])
        # # feats[1] = self.a4(feats[1], x5.sigmoid(), e5.sigmoid(), e50)
        # # x = self.d4(self.add4(x, feats[1]))
        # # x = x + self.a4(feats[1], x5.sigmoid(), e5.sigmoid())
        # # x = self.add4(torch.cat((x, self.a4(feats[1], x5.sigmoid(), e5.sigmoid())), 1))
        # # x = self.add4((x + self.a4(feats[1], x5.sigmoid(), e5.sigmoid())))
        # # x = self.add4(torch.cat((feats[1], self.a4(x, x5.sigmoid(), e5.sigmoid(), e50)), 1))
        # # x = self.a4(feats[1], x5.sigmoid(), x, e5.sigmoid(), e50, x5_bp)
        # # x = self.a4(self.add4(torch.cat((feats[1], x), 1)), x5.sigmoid(), x, e5.sigmoid(), e50)
        # # x = self.a4(self.add4(torch.cat((feats[1], x), 1)), x5.sigmoid())
        # # x = self.add4(torch.cat((feats[1], x), 1))
        # # x = self.d4(x)
        # # x = self.a4((feats[1] + x), x5.sigmoid(), e5.sigmoid())
        # # x = self.a4(torch.cat((feats[1], x), 1), x5.sigmoid(), e5.sigmoid())
        # # x = self.a4((feats[1] * x), x5.sigmoid(), e5.sigmoid())
        # # x = self.a4(self.d4(self.add4(feats[1], x)), x5.sigmoid(), e5.sigmoid())
        # # x = self.a4((self.aa4(feats[1], x5.sigmoid()) + x), x5.sigmoid())
        # # x4 = cus_sample(self.out_layer_04(x), mode="scale", factors=2)
        # x = cus_sample(x, mode="scale", factors=2)
        # # e4 = cus_sample(e5, mode="scale", factors=2)
        # x40 = cus_sample(x5, mode="scale", factors=2)
        #
        # # m4 = x
        # # x = self.a3(self.add3(torch.cat((feats[2], x), dim=1)), x40.sigmoid(), e4.sigmoid(), e40)
        # # x = self.a3((feats[2] + x), x40.sigmoid(), e4.sigmoid(), e40)
        # # x = self.add3((x + self.a3(feats[2], x40.sigmoid(), e4.sigmoid(), e40)))
        # x = self.add3(torch.cat((x, self.a3(feats[2], x40.sigmoid(), e4.sigmoid(), e40)), 1))
        # # x = self.add3(torch.cat((x, self.a3(feats[2], x40.sigmoid())), 1))
        # # x = self.add3(torch.cat((x, feats[2]), 1))
        # # x = self.add3(feats[2], x)
        # # x = self.d3(x)
        # x4 = self.out_layer_04(x)
        # # e4 = self.out_layer_04e(x)
        # # x4 = x4 * e4
        # # x4 = self.bout_layer_03(torch.cat((x4, e4), 1))
        # # e4 = cus_sample(edge, mode="size", factors=x4.shape[2:])
        # # e4 = self.mout_layer_02(feats[2])
        # # feats[2] = self.a3(x, x4.sigmoid(), e4.sigmoid(), e40)
        # # x = self.d3(self.add3(x, feats[2]))
        # # x = x + self.a3(feats[2], x4.sigmoid(), e4.sigmoid())
        #
        # # x = self.add3(torch.cat((x, self.a3(feats[2], x4.sigmoid(), e4.sigmoid())), 1))
        # # x = self.add3((x + self.a3(feats[2], x4.sigmoid(), e4.sigmoid())))
        # # x = self.add3(torch.cat((feats[2], self.a3(x, x4.sigmoid(), e4.sigmoid(), e40)), 1))
        # # x = self.a3(feats[2], x4.sigmoid(), x, e4.sigmoid(), e40, x4_bp)
        # # x = self.a3(self.add3(torch.cat((feats[2], x), 1)), x4.sigmoid(), x, e4.sigmoid(), e40)
        # # x = self.a3(self.add3(torch.cat((feats[2], x), 1)), x4.sigmoid())
        # # x = self.add3(torch.cat((feats[2], x), 1))
        # # x = self.d3(x)
        # # x = self.a3((feats[2] + x), x4.sigmoid(), e4.sigmoid())
        # # x = self.a3(torch.cat((feats[2], x), 1), x4.sigmoid(), e4.sigmoid())
        # # x = self.a3((feats[2] * x), x4.sigmoid(), e4.sigmoid())
        # # x = self.a3(self.d3(self.add3(feats[2], x)), x4.sigmoid(), e4.sigmoid())
        # # x = self.a3((self.aa3(feats[2], x4.sigmoid()) + x), x4.sigmoid())
        # # x3 = cus_sample(self.out_layer_03(x), mode="scale", factors=2)
        # x = cus_sample(x, mode="scale", factors=2)
        # # e3 = cus_sample(e4, mode="scale", factors=2)
        # x30 = cus_sample(x4, mode="scale", factors=2)
        #
        # # m3 = x
        # # x = self.a2(self.add2(torch.cat((feats[3], x), dim=1)), x30.sigmoid(), e3.sigmoid(), e30)
        # # x = self.a2((feats[3] + x), x30.sigmoid(), e3.sigmoid(), e30)
        # # x = self.add2((x + self.a2(feats[3], x30.sigmoid(), e3.sigmoid(), e30)))
        # x = self.add2(torch.cat((x, self.a2(feats[3], x30.sigmoid(), e3.sigmoid(), e30)), 1))
        # # x = self.add2(torch.cat((x, self.a2(feats[3], x30.sigmoid())), 1))
        # # x = self.add2(torch.cat((x, feats[3]), 1))
        # # x = self.add2(feats[3], x)
        # # x = self.d2(x)
        # x3 = self.out_layer_03(x)
        # # e3 = self.out_layer_03e(x)
        # # x3 = x3 * e3
        # # x3 = self.bout_layer_02(torch.cat((x3, e3), 1))
        # # e3 = cus_sample(edge, mode="size", factors=x3.shape[2:])
        # # e3 = self.mout_layer_03(feats[3])
        # # feats[3] = self.a2(x, x3.sigmoid(), e3.sigmoid(), e30)
        # # x = self.d2(self.add2(x, feats[3]))
        # # x = x + self.a3(feats[3], x3.sigmoid(), e3.sigmoid())
        # # x = self.add2(torch.cat((x, self.a2(feats[3], x3.sigmoid(), e3.sigmoid())), 1))
        # # x = self.add2((x + self.a2(feats[3], x3.sigmoid(), e3.sigmoid())))
        # # x = self.add2(torch.cat((feats[3], self.a2(x, x3.sigmoid(), e3.sigmoid(), e30)), 1))
        # # x = self.a2(feats[3], x3.sigmoid(), x, e3.sigmoid(), e30, x3_bp)
        # # x = self.a2(self.add2(torch.cat((feats[3], x), 1)), x3.sigmoid(), x, e3.sigmoid(), e30)
        # # x = self.a2(self.add2(torch.cat((feats[3], x), 1)), x3.sigmoid())
        # # x = self.add2(torch.cat((feats[3], x), 1))
        # # x = self.d2(x)
        # # x = self.a2((feats[3] + x), x3.sigmoid(), e3.sigmoid())
        # # x = self.a2(torch.cat((feats[3], x), 1), x3.sigmoid(), e3.sigmoid())
        # # x = self.a2((feats[3] * x), x3.sigmoid(), e3.sigmoid())
        # # x = self.a2(self.d2(self.add3(feats[3], x)), x3.sigmoid(), e3.sigmoid())
        # # x = self.a2((self.aa2(feats[3], x3.sigmoid()) + x), x3.sigmoid())
        # # x2 = cus_sample(self.out_layer_02(x), mode="scale", factors=2)
        # x = cus_sample(x, mode="scale", factors=2)
        # # e2 = cus_sample(e3, mode="scale", factors=2)
        # x20 = cus_sample(x3, mode="scale", factors=2)
        #
        # # m2 = x
        # # x = self.a1(self.add1(torch.cat((feats[4], x), dim=1)), x20.sigmoid(), e2.sigmoid(), e20)
        # # x = self.a1((feats[4] + x), x20.sigmoid(), e2.sigmoid(), e20)
        # # x = self.add1((x + self.a1(feats[4], x20.sigmoid(), e2.sigmoid(), e20)))
        # x = self.add1(torch.cat((x, self.a1(feats[4], x20.sigmoid(), e2.sigmoid(), e20)), 1))
        # # x = self.add1(torch.cat((x, self.a1(feats[4], x20.sigmoid())), 1))
        # # x = self.add1(torch.cat((x, feats[4]), 1))
        # # x = self.add1(feats[4], x)
        # # x = self.d1(x)
        # x2 = self.out_layer_02(x)
        # # e2 = self.out_layer_02e(x)
        #
        # # x2 = x2 * e2
        # # x2 = self.bout_layer_01(torch.cat((x2, e2), 1))
        # # e2 = cus_sample(edge, mode="size", factors=x2.shape[2:])
        # # e2 = self.mout_layer_04(feats[4])
        # # feats[4] = self.a1(x, x2.sigmoid(), e2.sigmoid(), e20)
        # # x = self.d1(self.add1(x, feats[4]))
        # # x = x + self.a2(feats[4], x2.sigmoid(), e2.sigmoid())
        # # x = self.add1(torch.cat((x, self.a1(feats[4], x2.sigmoid(), e2.sigmoid())), 1))
        # # x = self.add1((x + self.a1(feats[4], x2.sigmoid(), e2.sigmoid())))
        # # x = self.add1(torch.cat((feats[4], self.a1(x, x2.sigmoid(), e2.sigmoid(), e20)), 1))
        # # x = self.a1(feats[4], x2.sigmoid(), x, e2.sigmoid(), e20, x2_bp)
        # # x = self.a1(self.add1(torch.cat((feats[4], x), 1)), x2.sigmoid(), x, e2.sigmoid(), e20)
        # # x = self.a1(self.add1(torch.cat((feats[4], x), 1)), x2.sigmoid())
        # # x = self.add1(torch.cat((feats[4], x), 1))
        # # x = self.d1(x)
        # # x = self.a1((feats[4] + x), x2.sigmoid(), e2.sigmoid())
        # # x = self.a1(torch.cat((feats[4], x), 1), x2.sigmoid(), e2.sigmoid())
        # # x = self.a1((feats[4] * x), x2.sigmoid(), e2.sigmoid())
        # # x = self.a1(self.d1(self.add1(feats[4], x)), x2.sigmoid(), e2.sigmoid())
        # # x = self.a1((self.aa1(feats[4], x2.sigmoid()) + x), x2.sigmoid())
        #
        # x = cus_sample(x, mode="scale", factors=2)
        # e1 = cus_sample(e2, mode="scale", factors=2)
        # x10 = cus_sample(x2, mode="scale", factors=2)
        # # x = self.add0(x)
        # # x = self.ncd(feats[0], feats[1], feats[2], feats[3], feats[4])
        # # x1 = x
        # x = self.a0(x, x10.sigmoid(), e1.sigmoid())
        # x1 = self.out_layer_01(x)
        # # x1 = cus_sample(x1, mode="scale", factors=2)



        # feats1[0] = self.m6(feats1[0])
        x6_bp = self.add6b(feats1[0])
        x6_b = self.out_layer_06b(x6_bp)

        # x6_m = self.m6(feats2[0] + feats1[0])
        # x6_m = self.m6(torch.cat((feats1[0], feats2[0]), 1))
        # x6_m = self.m6(feats2[0], feats1[0], feats1[0], feats2[0])

        # x6_f = self.out_layer_06f(x6_m)
        # x6_f = self.mm6(torch.cat((x6_b, x6_f), 1))

        x6_bu = cus_sample(feats1[0], mode="scale", factors=2)
        # x6_fu = cus_sample(x6_m, mode="scale", factors=2)

        x5_bp = self.add5b(torch.cat((x6_bu, feats1[1]), 1))
        x5_b = self.out_layer_05b(x5_bp)

        # x5_m = self.m5(feats2[1] + x5_bp, x6_fu)
        # x5_m = self.m5(torch.cat((x5_bp, feats2[1], x6_fu), 1))
        # x5_m = self.m5(feats2[1], x5_bp, x5_bp, feats2[1])

        # x5_fp = self.add5f(torch.cat((x6_fu, feats2[1]), 1))
        # x5_m = self.m5(torch.cat((x5_fp, x5_bp), 1))
        # x5_fp = x5_m
        # x5_f = self.out_layer_05f(x5_m)
        # x5_f = self.mm5(torch.cat((x5_b, x5_f), 1))

        x5_bu = cus_sample(x5_bp, mode="scale", factors=2)
        # x5_fu = cus_sample(x5_m, mode="scale", factors=2)

        x4_bp = self.add4b(torch.cat((x5_bu, feats1[2]), 1))
        x4_b = self.out_layer_04b(x4_bp)

        # x4_m = self.m4(x4_bp, feats2[2] + x4_bp, x5_fu)
        # x4_m = self.m4(torch.cat((x4_bp, feats2[2], x5_fu), 1))
        # x4_m = self.m4(feats2[2], x4_bp, x4_bp, feats2[2])

        # x4_fp = self.add4f(torch.cat((x5_fu, feats2[2]), 1))
        # x4_m = self.m4(torch.cat((x4_fp, x4_bp), 1))
        # x4_fp = x4_m
        # x4_f = self.out_layer_04f(x4_m)
        # x4_f = self.mm4(torch.cat((x4_b, x4_f), 1))

        x4_bu = cus_sample(x4_bp, mode="scale", factors=2)
        # x4_fu = cus_sample(x4_m, mode="scale", factors=2)

        x3_bp = self.add3b(torch.cat((x4_bu, feats1[3]), 1))
        x3_b = self.out_layer_03b(x3_bp)

        # x3_m = self.m3(x3_bp, feats2[3] + x3_bp, x4_fu)
        # x3_m = self.m3(torch.cat((x3_bp, feats2[3], x4_fu), 1))
        # x3_m = self.m3(feats2[3], x3_bp, x3_bp, feats2[3])

        # x3_fp = self.add3f(torch.cat((x4_fu, feats2[3]), 1))
        # x3_m = self.m3(torch.cat((x3_fp, x3_bp), 1))
        # x3_fp = x3_m
        # x3_f = self.out_layer_03f(x3_m)
        # x3_f = self.mm3(torch.cat((x3_b, x3_f), 1))

        x3_bu = cus_sample(x3_bp, mode="scale", factors=2)
        # x3_fu = cus_sample(x3_m, mode="scale", factors=2)

        x2_bp = self.add2b(torch.cat((x3_bu, feats1[4]), 1))
        x2_b = self.out_layer_02b(x2_bp)

        # x2_m = self.m2(x2_bp, feats2[4] + x2_bp, x3_fu)
        # x2_m = self.m2(torch.cat((x2_bp, feats2[4], x3_fu), 1))
        # x2_m = self.m2(feats2[4], x2_bp, x2_bp, feats2[4])

        # x2_fp = self.add2f(torch.cat((x3_fu, feats2[4]), 1))
        # x2_m = self.m2(torch.cat((x2_fp, x2_bp), 1))
        # x2_fp = x2_m
        # x2_f = self.out_layer_02f(x2_m)
        # x2_f = self.mm2(torch.cat((x2_b, x2_f), 1))

        # x2_bu = cus_sample(x2_bp, mode="scale", factors=2)
        # # x2_fu = cus_sample(x2_m, mode="scale", factors=2)
        #
        # x1_bp = self.add1b(x2_bu)
        # x1_b = self.out_layer_01b(x1_bp)
        #
        # x1_m = x2_fu
        # x1_m = self.m1(x2_fu + x1_bp)
        # x1_fp = self.add1f(x2_fu)
        # x1_m = self.m1(torch.cat((x1_bp, x1_fp), 1))
        # x1_m = self.m1(x2_fu, x1_bp, x1_bp, x2_fu)
        #
        # x1_fp = x1_m
        # x1_f = self.out_layer_01f(x1_m)
        # x1_f = self.mm1(torch.cat((x1_b, x1_f), 1))

        # feats2 = self.br1(feats1[0], x5_bp, x4_bp, x3_bp, x2_bp)
        # feats2 = [x6_bp, x5_bp, x4_bp, x3_bp, x2_bp]
        # feats1 = self.br1(feats1[0], feats1[1], feats1[2], feats1[3], feats1[4])

        x6_fp = self.add6f0(feats2[0])
        x6_f = self.out_layer_06f0(x6_fp)
        x6_fu0 = cus_sample(feats2[0], mode="scale", factors=2)

        x5_fp = self.add5f0(torch.cat((x6_fu0, feats2[1]), 1))
        x5_f = self.out_layer_05f0(x5_fp)
        x5_fu0 = cus_sample(x5_fp, mode="scale", factors=2)

        x4_fp = self.add4f0(torch.cat((x5_fu0, feats2[2]), 1))
        x4_f = self.out_layer_04f0(x4_fp)
        x4_fu0 = cus_sample(x4_fp, mode="scale", factors=2)

        x3_fp = self.add3f0(torch.cat((x4_fu0, feats2[3]), 1))
        x3_f = self.out_layer_03f0(x3_fp)
        x3_fu0 = cus_sample(x3_fp, mode="scale", factors=2)

        x2_fp = self.add2f0(torch.cat((x3_fu0, feats2[4]), 1))
        x2_f = self.out_layer_02f0(x2_fp)
        # x2_fu0 = cus_sample(x2_fp, mode="scale", factors=2)

        # x1_fp = self.add1f0(x2_fu0)
        # x1_f0 = self.out_layer_01f0(x1_fp)

        feats3 = [self.add6f1(torch.cat((x6_bp, x6_fp), 1)), self.add5f1(torch.cat((x5_bp, x5_fp), 1)),
                          self.add4f1(torch.cat((x4_bp, x4_fp), 1)), self.add3f1(torch.cat((x3_bp, x3_fp), 1)),
                          self.add2f1(torch.cat((x2_bp, x2_fp), 1))]
        # feats3 = self.br1(self.add6f1(torch.cat((x6_bp, x6_fp), 1)), self.add5f1(torch.cat((x5_bp, x5_fp), 1)),
        #           self.add4f1(torch.cat((x4_bp, x4_fp), 1)), self.add3f1(torch.cat((x3_bp, x3_fp), 1)),
        #           self.add2f1(torch.cat((x2_bp, x2_fp), 1)))

        # feats3 = self.br2((feats1[0] + feats2[0]), (x5_bp + x5_fp), (x4_bp + x4_fp), (x3_bp + x3_fp), (x2_bp + x2_fp))
        # feats3 = [(x6_fp + x6_bp), (x5_fp + x5_bp), (x4_fp + x4_bp), (x3_fp + x3_bp), (x2_fp + x2_bp)]

        x6_fp = self.add6f(feats3[0])
        # x6_f = self.out_layer_06f(feats3[0])
        x6_fu = cus_sample(x6_fp, mode="scale", factors=2)

        x5_fp = self.add5f(torch.cat((x6_fu, feats3[1]), 1))
        # x5_f = self.out_layer_05f(x5_fp)
        x5_fu = cus_sample(x5_fp, mode="scale", factors=2)

        x4_fp = self.add4f(torch.cat((x5_fu, feats3[2]), 1))
        # x4_f = self.out_layer_04f(x4_fp)
        x4_fu = cus_sample(x4_fp, mode="scale", factors=2)

        x3_fp = self.add3f(torch.cat((x4_fu, feats3[3]), 1))
        # x3_f = self.out_layer_03f(x3_fp)
        x3_fu = cus_sample(x3_fp, mode="scale", factors=2)

        x2_fp = self.add2f(torch.cat((x3_fu, feats3[4]), 1))
        # x2_f = self.out_layer_02f(x2_fp)
        x2_fu = cus_sample(x2_fp, mode="scale", factors=2)

        # x2_fu = cus_sample(feats3[4], mode="scale", factors=2)
        x1_fp = self.add1f(x2_fu)
        x1_f = self.out_layer_01f(x1_fp)

        # feats3[0] = self.d5(feats[0])
        # feats3[1] = self.d4(feats[1])
        # feats3[2] = self.d3(feats[2])
        # feats3[3] = self.d2(feats[3])
        # feats3[4] = self.d1(feats[4])

        # x6_b = self.out_layer_06b(feats1[0])
        # x5_b = self.out_layer_05b(feats1[1])
        # x4_b = self.out_layer_04b(feats1[2])
        # x3_b = self.out_layer_03b(feats1[3])
        # x2_b = self.out_layer_02b(feats1[4])
        #
        # feats2 = self.br2(feats1[0], feats1[1], feats1[2], feats1[3], feats1[4])
        #
        # x6_f = self.out_layer_06f(feats2[0])
        # x5_f = self.out_layer_05f(feats2[1])
        # x4_f = self.out_layer_04f(feats2[2])
        # x3_f = self.out_layer_03f(feats2[3])
        # x2_f = self.out_layer_02f(feats2[4])

        # feats3 = self.br1(feats2[0], (x5_bp + x5_fp), (x4_bp + x4_fp), (x3_bp + x3_fp), (x2_bp + x2_fp))
        # feats3 = [(feats1[0] + feats2[0]), (feats1[1] + feats2[1]), (feats1[2] + feats2[2]), (feats1[3] + feats2[3]), (feats1[4] + feats2[4])]
        # feats3 = [self.d5(feats[0]), self.d4(feats[1]), self.d3(feats[2]), self.d2(feats[3]), self.d1(feats[4])]
        # feats3 = feats
        # x6_f = self.out_layer_06f(feats3[0])
        # x6_fu = cus_sample(feats3[0], mode="scale", factors=2)
        #
        # x5_fp = self.add5f(torch.cat((x6_fu, feats3[1]), 1))
        # x5_f = self.out_layer_05f(x5_fp)
        # x5_fu = cus_sample(x5_fp, mode="scale", factors=2)
        ...
        # x2_fu = cus_sample(x2_fp, mode="scale", factors=2)
        #
        # # x2_fu = cus_sample(feats2[4], mode="scale", factors=2)
        # x1_fp = self.add1f(x2_fu)
        # x1_f = self.out_layer_01f(x1_fp)

        # return dict(seg=x1, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6)
        # return dict(seg=x1, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6), dict(seg2=e2, seg3=e3, seg4=e4, seg5=e5, seg6=e6)
        # return dict(seg=x1, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6), dict(seg2=x2_d, seg3=x3_d, seg4=x4_d, seg5=x5_d, seg6=x6_d)
        # return dict(seg=x1, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6), dict(edge=edge)
        # return dict(seg=x1, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6), dict(edge=edge), dict(seg2=x2_b, seg3=x3_b, seg4=x4_b, seg5=x5_b, seg6=x6_b)
        return dict(seg=x1_f, seg2=x2_f, seg3=x3_f, seg4=x4_f, seg5=x5_f, seg6=x6_f), dict(seg2=x2_b, seg3=x3_b, seg4=x4_b, seg5=x5_b, seg6=x6_b)
        # return dict(seg=x1_f, seg2=x2_f, seg3=x3_b, seg4=x4_f, seg5=x5_b, seg6=x6_f), dict(seg2=x2_b, seg3=x3_f,
        #                                                                                    seg4=x4_b, seg5=x5_f,
        #                                                                                    seg6=x6_b)
        # return dict(seg=x1_f, seg2=x2_f, seg3=x3_f, seg4=x4_f, seg5=x5_f, seg6=x6_f), dict(seg2=x2_f, seg3=x3_f,
        #                                                                                    seg4=x4_f, seg5=x5_f,
        #                                                                                    seg6=x6_f)
        # return dict(seg=x1_b, seg2=x2_b, seg3=x3_b, seg4=x4_b, seg5=x5_b, seg6=x6_b), dict(seg=x1_b, seg2=x2_b,
        #                                                                                    seg3=x3_b, seg4=x4_b,
        #                                                                                    seg5=x5_b, seg6=x6_b)
        # return dict(seg=x1_b), dict(seg=x1_b)
        # return dict(seg=x1, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6)
        # return dict(seg=logits, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6,), dict(edge=e1, edge2=e2, edge3=e3, edge4=e4, edge5=e5, edge6=e6)
        # return dict(seg=logits, seg2=x2, seg3=x3, seg4=x4, seg5=x5, seg6=x6, ), dict(edge=e2, edge3=e3, edge4=e4,
        #                                                                              edge5=e5, edge6=e6, back=b2, back3=b3, back4=b4,
        #                                                                              back5=b5, back6=b6)
        # return dict(seg=x1)

        # return dict(seg=x1), dict(edge=edge)

    def train_forward(self, data, **kwargs):
        # assert not {"image1.5", "image1.0", "image0.5", "mask"}.difference(set(data)), set(data)

        output, output3 = self.body(
